...
def generate_graph(df, terminal, date, hari_ini):
    ...
    plt.subplots(figsize=(15, 7), dpi= 60)
    sns.lineplot(x="TIME", y="ARR", data=df[6:], 
                    label=f"Est. Arrival Flight T{terminal}", marker='o', color='darkmagenta')
    plt.xlabel('Waktu'); plt.ylabel('Scheduled Arrival Flight')
    plt.title(f"Data Scheduled Arrival Flight Terminal {terminal} CGK\nPada hari {hari_ini}\n", fontsize = 15)
    plt.grid()
    ...
    # Convert the first matplotlib chart to a PNG image
    buffer = io.BytesIO()
